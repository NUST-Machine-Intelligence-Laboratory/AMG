...
class LATransformer(nn.Module):
    def __init__(self, model, lmbd ):
        ...
        self.avgpool = nn.AdaptiveAvgPool2d((self.part, self.model.embed_dim))
        self.dropout = nn.Dropout(p=0.5)
        self.lmbd = lmbd
        for i in range(self.part):
            name = 'classifier'+str(i)
            setattr(self, name, ClassBlock(self.model.embed_dim, self.class_num, droprate=0.5, relu=False, bnorm=True, num_bottleneck=256))
    ...
    def forward(self, x, compute_attn=False, compute_entro=False):
        ...
        for i in range(self.num_blocks):
            x = self.model.blocks[i](x, compute_taylor_attn=compute_attn, compute_head_entro=compute_entro)
        x = self.model.norm(x)
